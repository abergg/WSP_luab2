<?php
require 'resources/includes/db_conn.php'; //Hämtar allt som står i db_conn.php.
...
if ($pdo) {
    ...
    // Om post submit dvs lyckas lägg till ett inlägg kommer får du tillbaka 'Du har lyckats lägga upp ett inlägg' och om det inte lyckas så får du tillbaka 'Du har inte lyckats lägga upp ett inlägg'.
    if (isset($_POST['submit'])) {
        $user = $_POST['author']; //Vi anger variabeln user som får värdet från select author.
        $headline = $_POST['title']; //Anger variabeln headline som får värdet från selected title.
        $headline = trim($headline);

        $slug = slugify($headline); //Tar bort skumma tecken i headline.

        $text = $_POST['message']; //Variabeln text får värdet från select message.
        ...
        /**********************************************************/
        // Om du lyckas lägga till ett inlägg får du tillbaka 'Du har lyckats lägga upp ett inlägg', om det inte lyckas så får du tillbaka 'Du har inte lyckats lägga upp ett inlägg'.
        if($pdo->query($sql)) {
